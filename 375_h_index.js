...
A scientist has index h if h of their N papers have at least h citations each, and the other N - h
papers have no more than h citations each. If there are multiple possible values for h, the maximum value is used.
...
function getHIndex(list) {
  let i = 0;
  for (let citations of papers.sort((a,b) => b-a)) {
    if (citations > i)
      i++
    else break;
  }
...
let papers = [4, 0, 0, 2, 3];
console.log(getHIndex(papers));
